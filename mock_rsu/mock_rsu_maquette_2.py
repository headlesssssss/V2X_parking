...
ANOMALY_PROBA = 0.03       # ← réduit de 0.08 à 0.03 pour viser ~10% d'anomalies
RANDOM_SEED   = 42         # ← ajouté : seed pour reproductibilité du dataset

random.seed(RANDOM_SEED)   # résultats identiques à chaque exécution
...
def get_distance(place_id):
    ...
    # ── ANOMALIE TYPE 2 : oscillation rapide libre ↔ occupé ───────────────────
        ...
        s["state_counter"] += 1
        if s["state_counter"] > random.randint(5, 12):     # ← réduit de 10-25 à 5-12 cycles
            _reset_to_normal(place_id)
    ...
    # ── ANOMALIE TYPE 3 : valeur figée ────────────────────────────────────────
        ...
        if s["frozen_counter"] > random.randint(20, 40):    # ← réduit de 150-200 à 20-40 cycles (~1 min)
            _reset_to_normal(place_id)
...
csv_file   = open(CSV_FILE, "w", newline="", encoding="utf-8")  # ← 'w' au lieu de 'a' pour un dataset propre

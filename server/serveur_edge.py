...
class ServeurEdge:
    ...
    def check_business_rules(self, distance, place_id):
        ...
        distances = list(h["distances"])
        dist = float(distance)
        # CORRECTION : seuils alignés avec la portée réelle du HC-SR04
        # < 0.5 cm = capteur mort | > 400 cm = hors portée maximale HC-SR04
        if dist < 0.5 or dist > 400.0: return True
        # Saut brutal impossible physiquement (> 50 cm entre 2 mesures)
                ...
                for spot_id_str, map_data in self.spots_mapping.items():
                    if map_data["hardware_id"] == hardware_id:
                        ...
                        if distances_array and index < len(distances_array):
                            ...
                            if res["anomalie"]:
                                # CORRECTION : marquer la place comme ANOMALIE dans la LDM
                                self.etat_places[spot_id] = "ANOMALIE"
                                anomalies_detectees.append(spot_id)
                                self.log_evenement("ANOMALIE", spot_id, f"Distance: {dist}cm (Votes:{res['votes']}/3)")
                                print(f"⚠️  ANOMALIE place {spot_id} | dist={dist}cm | votes={res['votes']}/3")
                        break 
            
            self.sauvegarder_etat_direct()

            libres    = len([v for v in self.etat_places.values() if v == 'FREE'])
            occupees  = len([v for v in self.etat_places.values() if v == 'OCCUPIED'])
            anomalies = len([v for v in self.etat_places.values() if v == 'ANOMALIE'])
            print(f"🔄 Parking : {libres} libres | {occupees} occupées | {anomalies} anomalies")
